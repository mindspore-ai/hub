...
.. py:function:: mindspore_hub.load(name, *args, source='gitee', pretrained=True, force_reload=False, **kwargs)
    ...
    参数：
        ...
        - **args** (tuple) - 网络初始化的参数。
        - **source** (str，可选) - 是否将 `name` 解析为gitee模型URI、github模型URI或本地资源。默认值：``'gitee'``。
        - **pretrained** (bool，可选) - 是否加载预训练模型。 默认值：``True``。
        - **force_reload** (bool，可选) - 是否从 `url` 重新加载网络和ckpt。 默认值：``False``。
