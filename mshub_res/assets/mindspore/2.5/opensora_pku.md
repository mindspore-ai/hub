...
summary: OpenSora-PKU is a large video generation model for text-to-video generation
...
As introduced in the [Open-Sora Plan Arxiv paper](https://arxiv.org/abs/2412.00131), the hyper-parameters of each stage is summarized in the following table:
...
Running this command will generate reconstructed videos under the given `output_generated_video_dir`. You can then evaluate some common metrics (e.g., ssim, psnr) using the script under `opensora/eval/script`.
