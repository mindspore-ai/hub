...
summary: InstantMesh is used for 3D mesh generation from multiview images
...
Following the original paper, we used Blender to render multiview frames for a 3D object in `.obj` for training. Typically for overfitting, three 3D objects from the objaverse dataset are used. We rendered 5 arbitrary views for each object with the corresponding camera parameters extracted.
